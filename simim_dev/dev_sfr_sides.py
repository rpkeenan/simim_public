...
from simim.galprops.am import am_dfcat, double_schechter_gen
import simim.siminterface as sim

from astropy.cosmology import FlatLambdaCDM

# SMF parameters for SIDES model
...
# tell where the breakdow happends
...
class bethermin17_base():
    """Code to implement the SIDES forecast - as described in 
    Bethermin et al. 2017. Note that the current implementation
    does not include scatter when performing abundance matching"""
    def __init__(self,
                 sim='UniverseMachine-BolshoiPlanck',
        ...
        ----------
        sim : string
            Name of simulation - should correspond to a SimIM formatted halo catalog, used for 
            abundance matching.
        sm_comp : bool, default is False
            If set to True, will carry out abundance matching to determine the halo-stellar mass
            connection if the basic files cannot be loaded
        sm_haloprop : 'vmax', 'mass', or 'auto'
            Halo property to abundance match to stellar mass function - vmax for peak rotational
            velocity, mass for halo mass. auto will check if vmax is present in the catalog and 
            use this, or use mass otherwise. Ignored if sm_comp==False
        sm_halomassmin : float
            Minimum halo mass to consider when abundance matching in units of Msun, defaults to 
            10^10 Msun. Ignored if sm_comp==False
        sm_remake : bool
        ...
        sffrac_fq00, sffrac_gamma, sffrac_logmt0, sffrac_alpha1, sffrac_alpha2, sffrac_sigsf0, sffrac_beta1, sffrac_beta2 : float
            Parameters controling the fraction of galaxies considered star forming - Equation 2 
            of Bethermin 17
        sfms_m0, sfms_m1, sfms_a0, sfms_a1, sfms_a2 : float
            Parameters controlling the star forming main sequence parameterization - Equation 6
            of Bethermin 17        
        sfms_correztlowz : bool
            Toggle whether the main-sequence fudge factor is applied at z<1 (see Bethermin 17
            Appendix B)
        sfms_sigma, sfms_msoffset, sfms_sboffset, sf_sflimit : float
            Parameters controlling the assignment of SFRs, see Bethermin 17 section 2.5
        """

        paths = _paths._paths()

        self.sim = sim
        self.name = 'SIDES_'+sim

        if self.name not in paths.sfrs.keys():
            paths._newsfrpath(self.name)

        self.path = paths.sfrs[self.name]
    ...
    def b17_smfpars(self, z, cosmo):
        ...
        # Update pars for chosen cosmology
